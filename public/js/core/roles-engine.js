window.CGB_ROLES=(function(){
  ...
  function waitReady(timeoutMs){
    if(window.CGB_AUTH&&window.CGB_AUTH.whenReady) return window.CGB_AUTH.whenReady(timeoutMs);
    return new Promise(resolve=>{
      ...
      if(window.CGB_AUTH&&window.CGB_AUTH.onChange){
        let off=()=>{};
        off=window.CGB_AUTH.onChange(st=>{if(st&&st.ready){clearInterval(t);off();resolve(st)}});
      }
    });
  }

  async function loadMyRole(force){
    await waitReady(5000);
    const s=window.CGB_AUTH&&window.CGB_AUTH.state;
    const userId=s&&s.user&&s.user.id||"";
    if(!force&&ready&&resolvedUserId===userId) return myRole;
      ...
      if(error){
        ...
        if(data&&data.custom_role_id){
          try{
            ...
            if(cr){nextCustomRole=cr;nextPermissions=cr.permissions||{}}
          }catch(e){}
        } else if(nextRole){
          try{
            const {data:cr}=await s.client.from("custom_roles").select("*").eq("key",nextRole).maybeSingle();
            if(cr){nextCustomRole=cr;nextPermissions=cr.permissions||{}}
          }catch(e){}
        }
      }
    ...
    if(revision!==loadRevision||!current||!current.user||current.user.id!==userId) return null;
    myRole=nextRole;
    ...
    return myRole;
      ...
      if(revision===loadRevision) loadPromise=null;
    });
    return loadPromise;
  }
  ...
  async function setRole(userId,role,displayName,customRoleId){
    ...
    try{
      ...
      if(!error){
        if(s.user&&s.user.id===userId) await loadMyRole(true);
        return {ok:true,saved:(data&&typeof data==="object")?data:null};
      }
  ...
  return {loadMyRole,getMyRole,getMyCustomRole,getMyPermissions,isReady,can,isAdmin,isSS,isStaff,onChange,
